...
class EpsilonInsensitiveLoss:
    ...
    def __init__(self, epsilon = 0.0, norm = 1) -> None:
        """Class implementing the Epsilon Insensitive loss model.

        Args:
            epsilon (float, optional): Value of threshold epsilon, under which error is ignored. Defaults to 0.0.
            norm (int, optional): Norm to be applied to error. Defaults to 1.
        """
    ...
    def __call__(self, outputs, targets):
        ...
        error[error < self.eps] = 0.0

        return (error ** self.norm).mean()
    

class OneCycleLR:
    """ Implementation of One Cycle Learning Rate scheduler.
